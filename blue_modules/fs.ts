...
export const openSignedTransaction = async function (): Promise<string | false> {
  try {
    const [res] = await pick({
      type: Platform.OS === 'ios' ? ['org.groestlcoin.psbt', 'org.groestlcoin.psbt.txn', types.json] : [types.allFiles],
    });
...
export const showFilePickerAndReadFile = async function (): Promise<{ data: string | false; uri: string | false }> {
  try {
    const [pickedFile] = await pick({
      type:
        Platform.OS === 'ios'
          ? ['org.groestlcoin.psbt', 'org.groestlcoin.psbt.txn', 'org.groestlcoin.backup', types.plainText, types.json, types.images]
          : [types.allFiles],
    });
...
export const pickTransaction = async () => {
  const [res] = await pick({
    type: Platform.OS === 'ios' ? ['org.groestlcoin.psbt', 'org.groestlcoin.psbt.txn', types.plainText, types.json] : [types.allFiles],
  });
