...
/**
 * Worker that periodically checks Groestlcoin block height and refreshes widgets
...
class BlockHeightCheckWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        Log.i(TAG, "Starting Groestlcoin block height check")
        ...
        try {
            ...
            Log.i(TAG, "Current Groestlcoin block height: $currentHeight")

            // Check if block height has changed
            if (lastKnownHeight > 0 && currentHeight > lastKnownHeight) {
                Log.i(TAG, "New Groestlcoin block detected! Height changed from $lastKnownHeight to $currentHeight")
