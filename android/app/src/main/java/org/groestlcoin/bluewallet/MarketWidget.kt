...
class MarketWidget : AppWidgetProvider() {
    ...
    companion object {
        ...
        private const val SHARED_PREF_NAME = "group.org.groestlcoin.bluewallet123"
        private const val DEFAULT_CURRENCY = "USD"
        private const val KEY_LAST_ONLINE_STATUS = "market_widget_last_online_status"

        private val hardcodedPeers = listOf(
            ElectrumServer("electrum1.groestlcoin.org", 50002, true),
            ElectrumServer("electrum2.groestlcoin.org", 50002, true)
        )

        private suspend fun connectToElectrumServer(): Boolean {
            for (peer in hardcodedPeers) {
                repeat(3) { attempt ->
                    Log.d(TAG, "Attempting to connect to Electrum-GRS server: ${peer.host}:${peer.port}, Attempt: ${attempt + 1}")
                    val success = ElectrumClient().connect(peer, validateCertificates = true)
                    if (success) {
                        Log.i(TAG, "Successfully connected to Electrum-GRS server: ${peer.host}:${peer.port}")
                        return true
                    } else {
                        Log.w(TAG, "Failed to connect to Electrum-GRS server: ${peer.host}:${peer.port}, Attempt: ${attempt + 1}")
                    }
                }
            }
            Log.e(TAG, "Failed to connect to any Electrum-GRS server from the hardcoded list after 3 attempts each. Waiting 10 minutes before retrying.")
