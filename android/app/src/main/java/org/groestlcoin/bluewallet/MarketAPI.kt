...
object MarketAPI {
    ...
    suspend fun fetchPrice(context: Context, currency: String): String? {
        Log.i(TAG, "Fetching Groestlcoin price for currency: $currency")
    ...
    suspend fun fetchNextBlockFee(context: Context): String {
        ...
        try {
            ...
            try {
                ...
                Log.i(TAG, "Successfully calculated next block fee: $formattedFee gro/vB")
                return formattedFee
            } catch (e: Exception) {
    ...
    private fun calculateFeeFromHistogram(feeHistogram: JSONArray, targetBlocks: Int): Double {
        try {
            ...
            val result = median.coerceAtLeast(2.0) // Minimum 2 gro/vB

            Log.d(TAG, "Calculated median fee rate: $median, final rate: $result gro/vB")
            return result
