...
object MarketAPI {
    ...
    /**
     * Fetch the next block fee from Electrum-GRS servers with network awareness
    ...
    suspend fun fetchNextBlockFee(context: Context): String {
        ...
        electrumClient.setNetworkStatusListener(object : ElectrumClient.NetworkStatusListener {
            override fun onNetworkStatusChanged(isConnected: Boolean) {
                Log.d(TAG, "Electrum-GRS network status changed: ${if (isConnected) "Connected" else "Disconnected"}")
            }

            override fun onConnectionError(error: String) {
                Log.e(TAG, "Electrum-GRS connection error: $error")
            }

            override fun onConnectionSuccess() {
                Log.d(TAG, "Successfully connected to Electrum-GRS server")
            }
        })
        ...
        try {
            ...
            Log.d(TAG, "Attempting to connect to Electrum-GRS server for fee histogram")
            if (!electrumClient.connectToNextAvailable(validateCertificates = false)) {
                Log.e(TAG, "Failed to connect to any Electrum-GRS peer")
                return "!"
            }
            ...
            if (receivedData.isEmpty()) {
                Log.e(TAG, "Empty response from Electrum-GRS server when requesting fee histogram")
                return "!"
            }
            ...
            val duration = System.currentTimeMillis() - startTime
            Log.i(TAG, "Successfully fetched next block fee in ${duration}ms: $formattedFee gro/vB for block height $currentBlockHeight")
     ...
     * @return the fee rate in gro/vB that would get confirmed in the target number of blocks
     */
    private fun calculateFeeFromHistogram(feeHistogram: JSONArray, targetBlocks: Int): Double {
        try {
            ...
            if (targetBlocks == 1 && entries.isNotEmpty()) {
                ...
                Log.d(TAG, "Using highest fee rate for next block: $highestFee gro/vB")
                return highestFee
            } else {
                ...
                for (entry in entries) {
                    ...
                    if (cumulativeSize >= totalSize * 0.1) { // Top 10% of transactions
                        Log.d(TAG, "Using fee rate for blocks $targetBlocks: ${entry.first} gro/vB")
                        return entry.first
                    }
                }
                ...
                if (entries.isNotEmpty()) {
                    ...
                    Log.d(TAG, "Using median fee rate: $medianFee gro/vB")
                    return medianFee
                }
            }
        } catch (e: Exception) {
        ...
        Log.w(TAG, "Using default fallback fee: 5 gro/vB")
        return 5.0
    }
