...
class ElectrumClient {
    companion object {
        ...
        // Default list of Electrum-GRS servers to try
        val hardcodedPeers = listOf(
            ElectrumServer("electrum1.groestlcoin.org", 50001, false),
            ElectrumServer("electrum2.groestlcoin.org", 50001, false)
        )
    ...
    /**
     * Connect to the next available Electrum-GRS server with network checks
     */
    suspend fun connectToNextAvailable(
        ...
        Log.i(TAG, "Starting connection attempt to Electrum-GRS server. Server count: ${servers.size}")
        ...
        if (!connected) {
            Log.e(TAG, "Failed to connect to any Electrum-GRS server after ${totalDuration}ms. Last error: ${lastError?.message}")
            networkStatusListener?.onConnectionError("Failed to connect to any Electrum-GRS server: ${lastError?.message}")
        } else {
            Log.i(TAG, "Successfully connected to an Electrum-GRS server in ${totalDuration}ms")
        }
    ...
    suspend fun connect(
        ...
        try {
            ...
            Log.e(TAG, "Error connecting to Electrum-GRS server: ${e.javaClass.simpleName} - ${e.message}")
        ...
        try {
            ...
            if (socket == null || socket?.isConnected != true) {
                ...
                if (!connectToNextAvailable(validateCertificates = false)) {
                    Log.e(TAG, "Failed to connect to Electrum-GRS server for block height")
                    return@withContext -1
                }
            }
    ...
    /**
     * Send data to the connected Electrum-GRS server with network check
     */
    suspend fun send(data: ByteArray): Boolean = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            Log.e(TAG, "Error sending data to Electrum-GRS server: ${e.javaClass.simpleName} - ${e.message}")
    ...
    /**
     * Receive data from the connected Electrum-GRS server with timeout handling
     */
    suspend fun receive(): ByteArray = withContext(Dispatchers.IO) {
        Log.d(TAG, "Waiting to receive data from Electrum-GRS server")
        val startTime = System.currentTimeMillis()
        ...
        try {
            ...
            if (responseData.isNotEmpty()) {
                ...
                Log.w(TAG, "Received empty response from Electrum-GRS server")
            }

            return@withContext responseData
        } catch (e: Exception) {
            Log.e(TAG, "Error receiving data from Electrum-GRS server: ${e.javaClass.simpleName} - ${e.message}")
    ...
    /**
     * Close the connection to the Electrum-GRS server
     */
    fun close() {
        try {
            inputReader?.close()
            ...
            Log.e(TAG, "Error closing Electrum-GRS connection", e)
        } finally {
            inputReader = null
            outputStream = null
            socket = null
