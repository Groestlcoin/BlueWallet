...
const About: React.FC = () => {
  ...
  const handleOnXPress = useCallback(() => {
    Linking.openURL('https://x.com/GroestlcoinTeam');
  }, []);
  ...
  const handleOnRatePress = useCallback(async () => {
    try {
      if (Platform.OS === 'ios') {
        await Linking.openURL('https://apps.apple.com/us/app/grs-bluewallet/id1518766083');
      } else {
        await Linking.openURL('https://play.google.com/store/apps/details?id=org.groestlcoin.bluewallet');
      }
    } catch (error: any) {
