...
export class AbstractWallet {
  ...
  /**
   *
   * @returns {number} Available to spend amount, int, in gros
   */
  getBalance(): number {
  ...
  setSecret(newSecret: string): this {
    ...
    this.secret = newSecret.trim().replace('groestlcoin:', '').replace('GROESTLCOIN:', '');
    ...
    if (!this._derivationPath) {
      if (this.secret.startsWith('xpub')) {
        this._derivationPath = "m/44'/17'/0'"; // Assume default BIP44 path for legacy wallets
      } else if (this.secret.startsWith('ypub')) {
        this._derivationPath = "m/49'/17'/0'"; // Assume default BIP49 path for segwit wrapped wallets
      } else if (this.secret.startsWith('zpub')) {
        this._derivationPath = "m/84'/17'/0'"; // Assume default BIP84 for native segwit wallets
      }
    }
