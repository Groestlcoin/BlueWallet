...
import { ECPairInterface } from 'ecpairgrs/src/ecpair';
...
export class AbstractHDElectrumWallet extends AbstractHDWallet {
    ...
    for (const tx of txs) {
      ...
      for (const vout of tx.outputs) {
        ...
        if (
            ...
            (vout.scriptPubKey.addresses?.[0] && ownedAddressesHashmap[vout.scriptPubKey.addresses[0]])
        ) {
  ...
  createTransaction(
    ...
    for (const u of utxos) {
      ...
      if (this._getExternalAddressByIndex(0).startsWith('grs1')) {
        u.script = { length: 27 };
    ...
    for (const t of targets) {
      if (t.address && t.address.startsWith('grs1')) {
  ...
  /**
   * Creates Segwit Bech32 Groestlcoin address
   */
