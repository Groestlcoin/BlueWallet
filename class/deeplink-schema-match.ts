...
class DeeplinkSchemaMatch {
   ...
   *
   * @param event {{url: string}} URL deeplink as passed to app, e.g. `groestlcoin:grs1qh6tf004ty7z7un2v5ntu4mkf630545gvhs45u7?amount=666&label=Yo`
  ...
  /**
   * Extracts server from a deeplink like `bluewallet:setelectrumserver?server=electrum1.groestlcoin.org%3A443%3As`
  ...
  static isBitcoinAddress(address: string): boolean {
    address = address.replace('://', ':').replace('groestlcoin:', '').replace('GROESTLCOIN:', '').replace('groestlcoin=', '').split('?')[0];
  ...
  static isLightningInvoice(invoice: string): boolean {
    ...
    if (
      invoice.toLowerCase().startsWith('lightning:lng') ||
      invoice.toLowerCase().startsWith('lightning://lng') ||
      invoice.toLowerCase().startsWith('lng')
    ) {
  ...
  static isBothBitcoinAndLightning(url: string): TBothBitcoinAndLightning {
    if (url.includes('lightning') && (url.includes('groestlcoin') || url.includes('GROESTLCOIN'))) {
      const txInfo = url.split(/(groestlcoin:\/\/|GROESTLCOIN:\/\/|groestlcoin:|GROESTLCOIN:|lightning:|lightning=|groestlcoin=)+/);
      ...
      for (const [index, value] of txInfo.entries()) {
        try {
          ...
          if (value.startsWith('groestlcoin') || value.startsWith('GROESTLCOIN')) {
            btc = `groestlcoin:${txInfo[index + 1]}`;
  ...
  static bip21decode(uri?: string) {
    ...
    let replacedUri = uri;
    for (const replaceMe of ['GROESTLCOIN://', 'groestlcoin://', 'GROESTLCOIN:']) {
      replacedUri = replacedUri.replace(replaceMe, 'groestlcoin:');
    }
