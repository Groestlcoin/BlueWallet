...
describe('HDSegwitElectrumSeedP2WPKHWallet', () => {
  ...
  it('can import mnemonics and generate addresses and WIFs', async function () {
    ...
    assert.strictEqual(
      hd.getXpub(),
      'zpub6n6X5F7QEogTDXchPXXhrvDQ38D5JTFNFWhFrFyri3Sazo4x225nENMeN1kKs1cYbeZDtuDUXhDQepUkxjnAi67z2PJ4d33qo8Cu3JhC3UE',
    );

    let address = hd._getExternalAddressByIndex(0);
    assert.strictEqual(address, 'grs1q2yv6rhtw9ycqeq2rkch65sucf66ytwsdvfvuhz');
    assert.ok(hd.getAllExternalAddresses().includes('grs1q2yv6rhtw9ycqeq2rkch65sucf66ytwsdvfvuhz'));
