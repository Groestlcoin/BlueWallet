...
describe('import procedure', () => {
  ...
  it('can be stopped', async () => {
    const store = createStore();
    const { promise, stop } = startImport('KztVRmc2EJJBHi599mCdXrxMTsNsGy3NUjc3Fb3FFDSMYyM2skod', false, false, false, ...store.callbacks);
    stop();
  ...
  it('can import Legacy', async () => {
    const store = createStore();
    const { promise } = startImport('KztVRmc2EJJBHi599mCdXrxMTsNsGy3NUjc3Fb3FFDSMYyM2skod', false, false, false, ...store.callbacks);
    await promise;
  ...
  it('can import Bech32 Segwit', async () => {
    const store = createStore();
    const { promise } = startImport('L1T6FfKpKHi8JE6eBKrsXkenw34d5FfFzJUZ6dLs2utxkSwQHDyp', false, false, false, ...store.callbacks);
    await promise;
  ...
  it('can import Legacy/P2SH/Bech32 from an empty wallet', async () => {
    const store = createStore();
    const { promise } = startImport('L36mabzoQyMZoHHsBFVNB7PUBXgXTynwY6yR7kYZ82EkS7q2o3ks', false, false, false, ...store.callbacks);
    await promise;
    assert.strictEqual(store.state.wallets[0].type, SegwitBech32Wallet.type);
  ...
  it('can import Legacy with uncompressed pubkey', async () => {
    const store = createStore();
    const { promise } = startImport('5KE6tf9vhYkzYSbgEL6M7xvkY69GMFHF3WxzYaCFMvwMxmNkoiD', false, false, false, ...store.callbacks);
    await promise;
  ...
  it('can import watch-only address', async () => {
    ...
    const { promise: promise1 } = startImport('FesL5FwNATk2yHryY4tq8CuyNc6p3ctd6S', false, false, false, ...store1.callbacks);
    await promise1;
    assert.strictEqual(store1.state.wallets[0].type, WatchOnlyWallet.type);

    const store2 = createStore();
    const { promise: promise2 } = startImport('3EoqYYp7hQSHn5nHqRtWzkgqmK3cYrMztd', false, false, false, ...store2.callbacks);
    await promise2;
    assert.strictEqual(store2.state.wallets[0].type, WatchOnlyWallet.type);

    const store3 = createStore();
    const { promise: promise3 } = startImport('grs1q8j4lk4qlhun0n7h5ahfslfldc8zhlxgywcavtt', false, false, false, ...store3.callbacks);
    await promise3;
